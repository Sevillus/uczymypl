...
const StudentsBox = ({student, setAddNewStudentMenu, fetchStudent, addNewStudentMenu}) => {
  ...
  return (
    <div className={"flex-between padding-y border-b-2  lg:pr-10"}>
      ...
      <p className={"w-56   text-start lg:flex  hidden"}>{student.email ? student.email : " - "}</p>
      <p className={"w-28 text-start lg:flex  hidden"}>{student.phone ? student.phone : " - "}</p>
      <p className={"w-28 text-start lg:flex hidden "}>{student.joinDate ? dayjs(student.joinDate).format("DD.MM.YYYY") : " - "}</p>
      <p className={"flex-center gap-2  w-28"}>
        ...
        <StudentsPageMoreInfo student={student} showMoreStudentInfo={showMoreStudentInfo} setShowMoreStudentInfo={setShowMoreStudentInfo}/>
    </div>
  );
};
